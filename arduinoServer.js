...
app.get("/", (req, res) => {
  pool.query(`SELECT BPM FROM data`, (err, rows, fields) => {
    ...
    const mean =
      heartRates.reduce((total, rate) => total + rate, 0) / heartRates.length;
    const sortedRates = heartRates.sort();
    const middleIndex = Math.floor(sortedRates.length / 2);
    const median =
      sortedRates.length % 2 === 0
        ? (sortedRates[middleIndex] + sortedRates[middleIndex - 1]) / 2
        : sortedRates[middleIndex];
    ...
    const modeV = modes.length === heartRates.length ? "No mode" : modes[-1]; //.join(", ");
    const values = [
      {
        mean: mean,
        median: median,
